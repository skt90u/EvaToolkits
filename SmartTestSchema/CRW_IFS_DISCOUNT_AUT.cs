//------------------------------------------------------------------------------
// <auto-generated>
//    這個程式碼是由範本產生。
//
//    對這個檔案進行手動變更可能導致您的應用程式產生未預期的行為。
//    如果重新產生程式碼，將會覆寫對這個檔案的手動變更。
// </auto-generated>
//------------------------------------------------------------------------------

namespace SmartTestSchema
{
    using System;
    using System.Collections.Generic;
    
    public partial class CRW_IFS_DISCOUNT_AUT
    {
        public string DISCOUNT_TYPE { get; set; }
        public string DISCOUNT_DESC { get; set; }
        public Nullable<decimal> DISCOUNT { get; set; }
        public string STATUS { get; set; }
        public string CRT_ID { get; set; }
        public string CRT_DATE { get; set; }
        public string CRT_TIME { get; set; }
        public string UPD_ID { get; set; }
        public string UPD_DATE { get; set; }
        public string UPD_TIME { get; set; }
        public string START_DATE { get; set; }
        public string END_DATE { get; set; }
        public Nullable<int> COND_AMT { get; set; }
        public string COND_CURY { get; set; }
        public string CREDIT_CARD_TYPE { get; set; }
        public string COUNTRY_CODE { get; set; }
        public Nullable<short> COND_QTY { get; set; }
        public Nullable<short> DISCOUNT_QTY { get; set; }
        public string PREORD_MARK { get; set; }
        public string AUT_DATE { get; set; }
        public string TRANS_NO { get; set; }
    }
}
