//------------------------------------------------------------------------------
// <auto-generated>
//    這個程式碼是由範本產生。
//
//    對這個檔案進行手動變更可能導致您的應用程式產生未預期的行為。
//    如果重新產生程式碼，將會覆寫對這個檔案的手動變更。
// </auto-generated>
//------------------------------------------------------------------------------

namespace SmartTestSchema
{
    using System;
    using System.Collections.Generic;
    
    public partial class UNI_CREW_APPLY_DETAILS
    {
        public string APPLY_NO { get; set; }
        public string CODE { get; set; }
        public string REAL_CODE { get; set; }
        public string CODE_DESC { get; set; }
        public Nullable<int> APPLY_QTY { get; set; }
        public Nullable<int> ACT_QTY { get; set; }
        public Nullable<int> RECEIVED_QTY { get; set; }
        public Nullable<decimal> PRICE { get; set; }
        public string IS_ORDER { get; set; }
        public string IS_CLOSE { get; set; }
        public string IS_OVER { get; set; }
        public string REMARK { get; set; }
        public string STATUS { get; set; }
        public string CRT_ID { get; set; }
        public string CRT_DATE { get; set; }
        public string CRT_TIME { get; set; }
        public string UPD_ID { get; set; }
        public string UPD_DATE { get; set; }
        public string UPD_TIME { get; set; }
        public Nullable<int> REUSE_QTY { get; set; }
        public Nullable<short> REASON_SEQ { get; set; }
        public string APPLY_REASON { get; set; }
    }
}
