...
public abstract class plUploadBaseHandler : IHttpHandler
{
    ...
    //private string InvalidFileExtensionUploaded = "InvalidFileExtensionUploaded";
    ...
    public void ProcessRequest(HttpContext context)
    {
        ...
        try
        {
            ...
            if (Request.Files.Count > 0)
            {
                ...
                int chunks = -1;
                if (!int.TryParse(Request["chunks"] ?? string.Empty, out chunks))
                    chunks = -1;

                int chunk = -1;
                if (!int.TryParse(Request["chunk"] ?? string.Empty, out chunk))
                    chunk = -1;
                ...
                if (chunks == -1)
                {
                    ...
                    {
                        string error = string.Format("上傳檔案({0})，超過可容許大小", fileName);
                        WriteErrorResponse(error, 413);
                        return;
                    }
                ...
                else
                {
                    ...
                    {
                        string error = string.Format("上傳檔案({0})，超過可容許大小({1})", fileName, MaxUploadSize);
                        WriteErrorResponse(error, 413);
                        return;
                    }
                ...
                {
                    string error = string.Format("上傳檔案({0})不在允許檔案類型({1})中", fileName, AllowedExtensions);
        ...
        {
            string error = string.Format("{0}, StackTrace: {1}", ex.Message, ex.StackTrace.ToString());
            WriteErrorResponse(error, 500);
            return;
        }
    }
